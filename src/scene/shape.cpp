...
RayIntersection Sphere::intersect(const Ray &ray) {
    ...
    float tCentre = glm::dot(L, ray.direction);
    float d = glm::dot(L, L) - tCentre*tCentre;
    float tIntersectToCentre = glm::sqrt(m_radius*m_radius-d);
    float t0 = tCentre-tIntersectToCentre;
    float t1 = tCentre+tIntersectToCentre;
