...
vec3 CorePathTracer::sampleRay(const Ray &ray, int) {
    ...
    if (intersect.m_valid) {
        for(size_t i = 0; i<m_scene->lights().size(); i++){
          //ambient light
           diffuse += m_scene->lights()[i]->ambience() * intersect.m_material->diffuse(); //update diffuse
            //if not occluded
            if(!m_scene->lights()[i]->occluded(m_scene, intersect.m_position)){
                irradiance = m_scene->lights()[i]->irradiance(intersect.m_position);
                incident = m_scene->lights()[i]->incidentDirection(intersect.m_position);
                iNormal = intersect.m_normal;
                float angle = glm::max(0.f, dot(-incident, iNormal));
                
                //lambertian
                diffuse += irradiance*angle*intersect.m_material->diffuse();
                
                vec3 reflect = glm::reflect(ray.direction, iNormal);
                float angle2 = glm::max(0.f, dot(-incident, reflect));
                float as = pow(angle2, intersect.m_material->shininess());
                
               // phong specular
               specular += irradiance*as*intersect.m_material->specular();
            }
        }
